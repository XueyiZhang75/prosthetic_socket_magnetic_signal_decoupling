"""APMD fixed-force passive creep test.

Formal Experiment 2.3, using the original Stage J control logic.
...
# --- Fixed-force passive creep settings ---
N_TRIALS = 1
F_HOLDS = [(430, 4.30)]
HOLD_S = 180.0
...
def acquire_force_target(mark10, force, contact_pos, target_F_N, log,
    ...
    lower_pos = (
        None if D_SOFT_LIMIT_MM is None
    ...
    for i in range(1, ACQUIRE_MAX_ITERS + 1):
        ...
        if lower_pos is not None:
            next_pos = max(next_pos, lower_pos)
        next_pos = min(next_pos, upper_pos)
        try:
...
def main():
    ...
    print("\n" + "=" * 60)
    print("  APMD -- fixed-force passive creep test")
    print("=" * 60)
    print(f"  session        : {session_dir.name}")
    print(f"  trials         : {N_TRIALS}")
    print(f"  force holds    : {[(lab, f) for lab, f in F_HOLDS]}")
    print(f"  hold duration  : {HOLD_S:.0f} s per target")
    d_soft_text = "OFF" if D_SOFT_LIMIT_MM is None else f"{D_SOFT_LIMIT_MM:.2f} mm"
    print(f"  D_SOFT_LIMIT   : {d_soft_text}")
    ...
    print()
    print("Pre-flight:")
    print("  [ ] Sample (bag + magnet) on MLX, same setup as APMD Stage 2")
    ...
    try:
        ...
        sys.exit(f"\nMLX90393 required for APMD fixed-force passive creep. {exc}")
    ...
    try:
        ...
        assert_no_contact_live_tare(force, "APMD fixed-force passive creep pre-flight")
    except Exception as exc:
        mlx_ser.close()
        mark10.close()
        sys.exit(f"\nForce sensor required for APMD fixed-force passive creep. {exc}")
    ...
    log.write(f"# APMD fixed-force passive creep session {ts}\n")
        ...
        for trial in range(1, N_TRIALS + 1):
            ...
            for hold_label, F_target in F_HOLDS:
                ...
                csv_path = session_dir / (
                    f"fixed_force_passive_creep_{hold_label}_rep{trial}.csv"
                )
    ...
    print(f"\n=== APMD fixed-force passive creep done. Files in: {session_dir}")
    for p in sorted(session_dir.glob("fixed_force_passive_creep_*.csv")):
        print(f"  {p.name}")
