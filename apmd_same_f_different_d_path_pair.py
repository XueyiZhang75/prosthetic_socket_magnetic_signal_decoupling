"""APMD near-matched-force / different-displacement active path-pair test.

Purpose:
  This experiment actively constructs a path-pair comparison:
  ...
  If the two target-force states match in F but differ in d and B, they give
  direct evidence that path history can create a displacement-identifiable
  magnetic response at nearly matched normal force.

      j_d = dB / dd | F

  This is Experiment 2.4 in APMD_FORMAL_EXPERIMENT_DESIGN.md.
"""
...
# --- APMD Experiment 2.4 path settings ---
N_TRIALS = 3
F_TARGETS = [(180, 1.80)]
D_PRELOAD_EXTRA_MM = 0.30
D_PRELOAD_FIXED_MM = None
D_PRELOAD_MAX_MM = 4.50
D_SOFT_LIMIT_MM = 4.50
...
SUMMARY_WINDOW_MODE = "tail"
ROW_PRINT_EVERY_S = 5.0

# --- Target acquisition ---
FORCE_MATCH_TOL_N = 0.050
FORCE_MATCH_TOL_FRAC = 0.0
...
POSITION_BOUNDS_MARGIN_MM = 0.05
PRELOAD_STEP_MM = 0.10
PRELOAD_CHECK_SETTLE_S = 0.25
PRELOAD_CHECK_SAMPLE_S = 0.25
PRELOAD_MOVE_MAX_WAIT_S = 20.0
PRELOAD_MOVE_STALL_WINDOW_S = 3.0
PRELOAD_DEPTH_EPS_MM = 0.005
...
FORMAL_EXPERIMENT_KEY = "\u5b9e\u9a8c 2.4"
PROTOCOL_TITLE = (
    "APMD -- near-matched-force / different-displacement active path-pair test"
)
PROTOCOL_SHORT_NAME = "APMD same-F/different-d path-pair"
LOG_HEADER = "APMD same-F/different-d path-pair session"
SUMMARY_FILENAME = "same_f_different_d_pair_summary.csv"
FIGURE_FILENAME = "same_f_different_d_path_pair.png"
CSV_PREFIX = "same_f_different_d"
CSV_PRINT_GLOB = f"{CSV_PREFIX}*.csv"
NEXT_MESSAGE = "Next: plot with the APMD same-F/different-d path-pair script."
...
def force_entry_ok(error_N, tol_N, state_label):
    return abs(error_N) <= tol_N


def compute_preload_depth(loading_d_mm):
    if D_PRELOAD_FIXED_MM is not None:
        return D_PRELOAD_FIXED_MM
    if D_PRELOAD_MAX_MM is None:
        return loading_d_mm + D_PRELOAD_EXTRA_MM
    return min(loading_d_mm + D_PRELOAD_EXTRA_MM, D_PRELOAD_MAX_MM)


def preload_plan_description():
    if D_PRELOAD_FIXED_MM is not None:
        return f"fixed d={D_PRELOAD_FIXED_MM:.2f} mm"
    cap_text = "OFF" if D_PRELOAD_MAX_MM is None else f"{D_PRELOAD_MAX_MM:.2f} mm"
    return f"loading d + {D_PRELOAD_EXTRA_MM:.2f} mm, cap {cap_text}"


def preload_force_cap(match_target_N):
    if F_PRELOAD_CAP_EXTRA_N is None and F_PRELOAD_CAP_MAX_N is None:
        return None
    if F_PRELOAD_CAP_EXTRA_N is None:
        return F_PRELOAD_CAP_MAX_N
    cap = match_target_N + F_PRELOAD_CAP_EXTRA_N
    if F_PRELOAD_CAP_MAX_N is None:
        return cap
    return min(cap, F_PRELOAD_CAP_MAX_N)
...
def clamp_acquire_position(pos, lower_pos, upper_pos):
    return min(max(pos, lower_pos), upper_pos)
...
def acquire_force_target(
    ...
    *,
    ...
    """Move in the original Stage J+ bracket-and-pick style."""
    lower_pos = (
        float("-inf")
        if D_SOFT_LIMIT_MM is None
        else contact_pos - D_SOFT_LIMIT_MM
    )
    ...
    for i in range(1, ACQUIRE_MAX_ITERS + 1):
        ...
        if pos > upper_pos + POSITION_BOUNDS_MARGIN_MM or (
            lower_pos != float("-inf")
            and pos < lower_pos - POSITION_BOUNDS_MARGIN_MM
        ):
            log.write(
                f"[trial {trial} pair {pair_label} {state_label} acquire] "
                f"position_out_of_bounds pos={pos:.4f} "
                f"bounds=({lower_pos:.4f},{upper_pos:.4f}) "
                f"targetF={target_F_N:.4f} F={F_m:.4f} "
                f"d={d_actual:.4f}\n"
            )
            log.flush()
            return False, "position_out_of_bounds", pos, d_actual, F_m, i
        ...
        if force_entry_ok(err, tol, state_label):
            return True, "reached", pos, d_actual, F_m, i

        current = {"pos": pos, "d": d_actual, "F": F_m, "err": err}
        if previous is not None and err * previous["err"] < 0:
            best = (
                current
                if abs(current["err"]) <= abs(previous["err"])
                else previous
            )
            if abs(best["err"]) > tol:
                mid_pos = clamp_acquire_position(
                    0.5 * (previous["pos"] + current["pos"]),
                    lower_pos,
                    upper_pos,
                )
                log.write(
                    f"[trial {trial} pair {pair_label} {state_label} acquire] "
                    f"wide_bracket best_err={best['err']:.4f} "
                    f"tol={tol:.4f} mid_pos={mid_pos:.4f}\n"
                )
                log.flush()
                try:
                    pos = mark10.move_to(
                        mid_pos,
                        tolerance_mm=CONTROL_MOVE_TOL_MM,
                    )
        ...
        if (
            D_SOFT_LIMIT_MM is not None
            and err > 0
            and d_actual >= D_SOFT_LIMIT_MM
        ):
    ...
    *,
    ...
    while True:
        ...
        row = {
            ...
            "repeat_id": trial,
            "pair_id": pair_id,
            "stage": "apmd_same_f_different_d_path_pair",
...
def pair_diagnostics(loading, unloading):
    ...
    disp_ok = abs(dd) + MIN_D_SPLIT_EPS_MM >= MIN_D_SPLIT_MM
...
def move_to_preload_depth(
    ...
    """Move deeper using the original simple staged preload logic."""
    pos = mark10.position_stable()
    current_d = contact_pos - pos

    while current_d < preload_d_mm - CONTROL_MOVE_TOL_MM - 1e-9:
        next_d = min(current_d + PRELOAD_STEP_MM, preload_d_mm)
        target_pos = contact_pos - next_d
        try:
            pos = mark10.move_to(target_pos, tolerance_mm=CONTROL_MOVE_TOL_MM)
        except Mark10Error as exc:
        ...
        if force_cap_N is not None and F_m >= force_cap_N:
...
def run_pair(
    *,
    ...
    force_cap_text = "OFF" if force_cap is None else f"{force_cap:.3f} N"
    print(
        f"\n  PRELOAD: move to d={preload_d:.3f} mm "
        f"({preload_plan_description()}, cap F={force_cap_text})"
    )
    ok, status, pos, preload_actual_d, preload_F = (
        move_to_preload_depth(
            mark10, force, contact_pos, preload_d, force_cap, log=log)
    )
    print(
...
def main():
    ...
    expected_pair_count = N_TRIALS * len(F_TARGETS)
    ...
    preload_cap_text = (
        "OFF" if D_PRELOAD_MAX_MM is None else f"{D_PRELOAD_MAX_MM:.2f} mm"
    )
    depth_soft_text = (
        "OFF" if D_SOFT_LIMIT_MM is None else f"{D_SOFT_LIMIT_MM:.2f} mm"
    )
    print(
        f"  preload d      : {preload_plan_description()}"
    )
    print(f"  D_SOFT_LIMIT   : {depth_soft_text}")
    ...
    print()
    print("Pre-flight:")
    print("  [ ] Same sample / magnet / MLX geometry as APMD Stage 2")
    ...
    try:
        ...
        sys.exit(f"\nMLX90393 required for APMD same-F/different-d. {exc}")
    ...
    try:
        ...
        assert_no_contact_live_tare(force, "APMD same-F/different-d pre-flight")
    except Exception as exc:
        mlx_ser.close()
        mark10.close()
        sys.exit(f"\nForce sensor required for APMD same-F/different-d. {exc}")
    ...
    log.write(
        ...
        f"D_PRELOAD_FIXED_MM={D_PRELOAD_FIXED_MM}, "
    ...
    try:
        for trial in range(1, N_TRIALS + 1):
            ...
            for pair_id, (target_label_int, target_F) in enumerate(
                ...
                planned_depth_text = (
                    f"{contact_depth + D_PRELOAD_FIXED_MM:.2f} mm"
                    if D_PRELOAD_FIXED_MM is not None
                    else "not software-capped"
                    if D_PRELOAD_MAX_MM is None
                    else f"{contact_depth + D_PRELOAD_MAX_MM:.2f} mm"
                )
                print(
                    f"  contact depth from start = {contact_depth:.2f} mm; "
                    f"max planned travel from start = {planned_depth_text}"
                )

                csv_path = session_dir / (
                    f"{CSV_PREFIX}_{target_label_int}_rep{trial}.csv"
                )
                ...
                if status != "ok":
                    ...
                    if (
                        diag["same_F_ok"]
                        and diag["disp_split_ok"]
                        and diag["b_signal_ok"]
                    ):
                        usable_pair_count += 1
                    log.write(
        ...
        registry_note = (
            ...
            f"{pair_summary_count}/{expected_pair_count} pairs"
        )
    except Exception as exc:
