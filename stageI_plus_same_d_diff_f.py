"""Stage I+ -- same-displacement, different-force path probe.

Purpose:
  The first Stage I hold showed clear force relaxation but weak magnetic
  response at fixed d. Stage I+ creates a stronger same-d / different-F
  comparison using path history:
...
TARGET_RECORD_S = 30.0
PRELOAD_RECORD_S = 15.0
PRE_RECORD_SETTLE_S = 5.0
STREAM_DISCARD_S = 1.0
SUMMARY_WINDOW_S = 8.0
...
PROTOCOL_TITLE = "Stage I+ -- same-d / different-F path probe"
PROTOCOL_SHORT_NAME = "Stage I+"
STAGE_LABEL = "Iplus"
LOG_HEADER = "Stage I+ session"
SUMMARY_FILENAME = "Iplus_pair_summary.csv"
STATE_FILE_PREFIX = "Iplus_same_d"
CSV_PRINT_GLOB = "Iplus_*.csv"
PREFLIGHT_FIRST_LINE = (
    "Same sample / magnet / MLX geometry as the completed Stage I run"
)
NEXT_MESSAGE = "Next: python .\\plot_stage_I_plus.py"
...
def pair_diagnostics(direct, returned):
    ...
    same_d_ok = abs(d_diff) <= D_MATCH_TOL_MM
...
def main():
    ...
    est_total_min = (
        ...
        * (2 * TARGET_RECORD_S + PRELOAD_RECORD_S
           + 3 * PRE_RECORD_SETTLE_S + 3 * INTER_STATE_SETTLE_S + 90)
        / 60
    )
    ...
    try:
        ...
        sys.exit(f"\nMLX90393 required for {PROTOCOL_SHORT_NAME}. {exc}")
    ...
    try:
        ...
        mlx_ser.close()
        mark10.close()
        sys.exit(f"\nForce sensor required for {PROTOCOL_SHORT_NAME}. {exc}")
